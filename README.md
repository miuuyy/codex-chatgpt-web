...
- **Real parallel Ultra orchestration.** The manually selected Ultra route uses Extra High as its
  coordinator and may launch up to three isolated native Codex worker tasks. The bridge pins each
  worker to its configured native Codex model, forces a read-only sandbox, suppresses verbose child
  event logs, and returns only the worker task ID and final report while leaving the service tier at
  the user's default. Separately, the bridge can overlap up to four Ultra browser turns, each in its
  own Temporary Chat page. Normal routes stay exclusive.
...
> [!WARNING]
> **Ultra is experimental and explicitly opt-in.** It may fan out to three native Codex workers and
