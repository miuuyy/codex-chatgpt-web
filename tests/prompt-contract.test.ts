...
test("injects bounded project continuity into prompt when provided", () => {
  ...
  const compiledMultipartWithContinuity = compileChatGptWebPrompt(
    request("high"),
    { localToolsEnabled: true, solAvailable: true, proAvailable: true },
    "turn_12345678901234567890123456789012",
    { projectContinuity: continuity, experimentalMultipartParts: 3 },
  );
  expect(compiledMultipartWithContinuity.multipart?.commit).toContain("<project_continuity>");
  expect(compiledMultipartWithContinuity.multipart?.commit).toContain("An awesome project");
