...
> [!WARNING]
> 请先在启动器中选择一个唯一的连接器名称，再使用该准确名称创建**新**连接器，并将权限设置为
> **允许所有操作**。不要重命名、刷新或复用旧的 **Codex Native** 连接器：ChatGPT 会按连接器身份缓存公开 MCP 合约，而
...
3. 选择一个唯一的连接器名称，粘贴 Tunnel ID 和 API 密钥，然后点击 **连接 Harness**。
4. 在 ChatGPT 设置中启用 **开发者模式**。新建连接器时选择 **Tunnel**，选择刚创建的
   Tunnel，将 **身份验证** 设为 **无**，并使用启动器中显示的准确连接器名称。
5. 如果已有旧的 **Codex Native** 连接器，请保持其不变。不要重命名或刷新它：ChatGPT 会按
   连接器身份缓存公开 MCP 合约，而此版本使用新的直接 turn-token 合约。在新连接器的 **权限**
   中选择 **允许所有操作**；**允许低风险操作** 会在命令和补丁到达本地运行时前将其
   拦截。外层 Codex harness 仍会执行沙箱和审批规则。
6. 运行 **验证运行时**。它只会准确选择已配置的连接器名称。如果只找到 **Codex Native**，
