...
contextBridge.exposeInMainWorld("codexWebLauncher", {
  ...
  pickCloudflareConfig: () => ipcRenderer.invoke("launcher:pick-cloudflare-config"),
  pickCloudflaredBinary: () => ipcRenderer.invoke("launcher:pick-cloudflared-binary"),
  setMcpStep: (step) => ipcRenderer.invoke("launcher:set-mcp-step", step),
  setMcpPreferences: (input) => ipcRenderer.invoke("launcher:set-mcp-preferences", input),
