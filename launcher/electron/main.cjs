...
const NATIVE_COPY = Object.freeze({
  ...
  "ru": Object.freeze({
    openLauncher: "Открыть Codex Web GPT",
    quit: "Завершить",
    exportDiagnostics: "Экспортировать безопасную для приватности диагностику",
    cancel: "Отмена",
    remove: "Удалить",
    removeTitle: "Удалить Codex Web GPT",
    removeMessage: "Удалить модели ChatGPT Web из Codex и восстановить предыдущий маршрут моделей?",
    removeDetail: "Профиль входа ChatGPT в лаунчере будет сохранён. Codex нужно перезапустить один раз.",
    retry: "Повторить",
    startupTitle: "Не удалось запустить Codex Web GPT",
    startupDetail: "Повторный запуск перезапустит лаунчер без изменения сохранённых настроек или профиля ChatGPT.",
    startupCleanupFailed: "Не удалось выполнить очистку при запуске",
    catalogFailure: "Codex подключился к лаунчеру, но загрузить каталог моделей не удалось (HTTP {status}; {reason}). Подробности смотрите в разделе «Активность»; если проблема сохраняется, экспортируйте безопасный лог.",
  }),
  "zh-CN": Object.freeze({
