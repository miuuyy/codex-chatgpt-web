...
const ru: Record<keyof typeof en, string> = {
  product: "Codex Web GPT",
  devBadge: "DEV",
  devSetupTitle: "Настройка изолированного DEV-профиля",
  devSetupSubtitle: "Этот браузер, аккаунт, конфигурация и данные среды выполнения хранятся отдельно от обычного лаунчера и Codex.",
  devCoreSetup: "DEV-профиль",
  devStepInstall: "Инициализировать DEV-среду",
  devStepInstallBody: "Сохранить возможности аккаунта в изолированном DEV-каталоге. Маршрут Codex, слушатель Responses и системная служба не устанавливаются; в полном режиме контролируется только изолированный MCP-туннель.",
  devInstall: "Инициализировать профиль",
  devReinstall: "Обновить профиль",
  devMcpTitle: "Симулированные инструменты через MCP",
  devMcpSubtitle: "Подключите отдельный аккаунт ChatGPT к изолированной среде репозитория.",
  devMcpBody: "DEV-лаунчер поддерживает изолированный MCP-туннель в готовом состоянии. Именованные чаты репозитория подключаются к нему и возвращают явные симулированные квитанции о действиях инструментов.",
  devConnectorIsolationNotice: "Создайте отдельный коннектор с точным DEV-именем ниже. Не изменяйте Codex Native2, чтобы production и разработка репозитория могли работать параллельно.",
  devSettingsTitle: "Настройки DEV-профиля",
  devKeepRunningBody: "Оставлять изолированную сессию браузера и DEV MCP-туннель доступными после закрытия этого окна.",
  biggerContext: "Расширенный контекст (экспериментально)",
  biggerContextBody: "Предупреждение: небольшие запросы остаются одним сообщением, а большой контекст делится на два или шесть сообщений. Последняя часть запускает задачу без дополнительного запроса. Лимиты контекста модели и compaction увеличиваются втрое; после изменения перезапустите Codex. Дополнительные запросы могут чаще приводить к rate limit или временной паузе. По умолчанию отключено.",
  skillAttachments: "Skills как файлы (экспериментально)",
  skillAttachmentsBody: "Загружать явно выбранные Codex skills как именованные текстовые файлы вместо встроенных инструкций. Остальные skills по-прежнему загружаются через инструменты. По умолчанию отключено; используется тот же лимит вложений, что и для изображений.",
  manualSkillAttachmentsUnavailable: "Загрузка skills требует автоматического взаимодействия с браузером.",
  biggerContextRecommendationTitle: "До 3× больше контекста",
  biggerContextRecommendationBody: "Расширенный контекст отправляет большие задачи несколькими сообщениями, чтобы ChatGPT Web мог использовать до трёх раз больше контекста. Функция остаётся экспериментальной и может чаще приводить к rate limit или временной паузе.",
  biggerContextRecommendationToggleBody: "Разбивать большие задачи на несколько сообщений и увеличивать лимиты контекста модели и compaction.",
  tagline: "ChatGPT Web внутри нативной среды Codex",
  chooseLanguage: "Выберите язык",
  chooseLanguageHint: "Позже язык можно изменить в настройках.",
  continue: "Продолжить",
  supportTitle: "Перед началом",
  supportBody: "Поставьте звезду репозиторию и откройте мой профиль в X, чтобы больше разработчиков узнали о Codex Web GPT. В X я публикую новости продукта и полезные советы по Codex.",
  star: "Поставить звезду на GitHub",
  starred: "GitHub открыт",
  follow: "Открыть мой X",
  followed: "X открыт",
  finishWelcome: "Открыть лаунчер",
  setup: "Настройка",
  activity: "Активность",
  settings: "Настройки",
  ...
  hideTab: "Закрыть вкладку",
  browserTabLimit: "До пяти одновременных вкладок ChatGPT Web. Ограничение помогает избежать избыточного параллельного трафика в вашем аккаунте ChatGPT.",
  browserAddress: "Браузер ChatGPT",
  noActiveTask: "Нет активной задачи",
  noActiveTaskBody: "ChatGPT появится здесь, когда Codex запустит запрос к Web-модели.",
  browserReady: "Браузер готов",
  showBrowser: "Показать ChatGPT",
  hideBrowser: "Скрыть ChatGPT",
  setupTitle: "Настройка Codex Web GPT",
  setupSubtitle: "Три проверки сделают ChatGPT Web доступным в нативном выборе моделей Codex.",
  coreSetup: "Основная настройка",
  interactionMode: "Взаимодействие с ChatGPT",
  interactionModeOnboardingBody: "Выберите, как лаунчер должен работать с ChatGPT. По умолчанию выбран режим «С автоматизацией». Позже его можно изменить в настройках.",
  automaticInteraction: "С автоматизацией",
  automaticInteractionBody: "Автоматически отправляет промпты и считывает состояние страницы ChatGPT. Контролируемые повторы предотвращают повторную отправку, но автоматизация браузера может конфликтовать с условиями OpenAI или политиками аккаунта.",
  manualInteraction: "Zero Risk",
  manualInteractionBody: "Никогда не читает и не изменяет ChatGPT. Лаунчер подготавливает промпт; вы сами вставляете его, выбираете коннектор, модель и уровень рассуждения и отправляете.",
  optional: "Необязательно",
  required: "Обязательно",
  stepAccount: "Войти в ChatGPT",
  stepAccountBody: "Войдите прямо во встроенном браузере ChatGPT. Сессия входа останется в приватном профиле этого лаунчера.",
  signIn: "Открыть вход",
  passkeySignIn: "Использовать passkey",
  passkeyContinue: "Продолжить",
  passkeyImporting: "Импорт…",
  passkeyContinueBody: "Завершите вход с passkey в отдельном окне Chrome, затем вернитесь сюда и нажмите «Продолжить».",
  checkingSignIn: "Проверка сохранённой сессии",
  verifySignIn: "Проверить вход",
  signedIn: "Вход выполнен",
  stepSmoke: "Запустить smoke-тест браузера",
  stepSmokeBody: "Выберите High, отправьте небольшое временное сообщение и убедитесь, что потоковый ответ завершился полностью.",
  runSmoke: "Запустить smoke-тест",
  smokePassed: "Smoke-тест пройден",
  stepInstall: "Установить в Codex",
  stepInstallBody: "Добавить модели ChatGPT Web, не заменяя нативный каталог Codex. Текущий пользовательский маршрут будет сохранён и восстановлен при отключении моста.",
  zeroRiskModelSettings: "Профили моделей Zero Risk",
  zeroRiskModelSettingsBody: "Выберите, какие строки моделей установить в Codex.",
  zeroRiskDefaultProfile: "По умолчанию",
  zeroRiskDefaultProfileBody: "Установить только ChatGPT Web — Zero Risk.",
  zeroRiskProProfile: "Pro",
  zeroRiskProProfileBody: "Также установить ChatGPT Web — Zero Risk Pro.",
  zeroRiskProProfileInfo: "Даёт примерно 250 000 доступных токенов до compaction. Для каждого запроса вручную выбирайте ChatGPT Pro. Требуется аккаунт ChatGPT Pro; без него запрос, скорее всего, завершится ошибкой, а увеличенный лимит может работать не со всеми уровнями рассуждения. Zero Risk не может проверить вашу подписку или выбор.",
  install: "Установить модели",
  reinstall: "Переустановить",
  awaitingCodex: "Перезапустите Codex",
  restartCodex: "Полностью завершите Codex, включая фоновый процесс, затем откройте его снова, чтобы обновить список моделей. Выход из аккаунта и повторный вход или простое закрытие окна не считаются перезапуском. Оставьте этот лаунчер открытым.",
  mcpTitle: "Нативные инструменты Codex через MCP",
  mcpSubtitle: "Подключите ChatGPT к активной среде Codex через туннель OpenAI.",
  mcpBody: "Разрешить всем доступным уровням ChatGPT Web, включая Pro, использовать активную среду Codex.",
  configureMcp: "Настроить MCP",
  mcpReady: "MCP настроен",
  close: "Закрыть",
  previous: "Назад",
  next: "Далее",
  done: "Готово",
  guideVideo: "Видеоинструкция",
  expandGuideVideo: "Развернуть видеоинструкцию",
  closeGuideVideo: "Закрыть развёрнутое видео",
  mcpStepOne: "Создайте туннель и API-ключ",
  mcpStepOneBody: "Создайте туннель OpenAI, скопируйте его Tunnel ID и создайте обычный API-ключ с правами Tunnels Read + Use (бесплатно; ключ нужен только для запуска туннеля). (Не забудьте создать рабочее пространство ChatGPT.)",
  openTunnels: "Открыть Tunnels",
  openKeys: "Создать API-ключ",
  mcpStepTwo: "Подключите локальную среду",
  mcpStepTwoBody: "Вставьте Tunnel ID и API-ключ. Туннель должен принадлежать тому же аккаунту OpenAI, который будет использовать плагин ChatGPT. Ключ хранится только в приватном локальном хранилище и никогда не записывается в логи лаунчера.",
  mcpStepTwoHint: "Добавить MCP-коннектор в ChatGPT можно только после успешного выполнения этого шага и запуска туннеля.",
  mcpCatalogRequired: "Подключение среды недоступно, пока модели Codex не установлены и не проверены. Вернитесь в «Настройку», нажмите «Установить модели», один раз перезапустите Codex и дождитесь завершения проверки каталога моделей.",
  tunnelId: "Tunnel ID",
  runtimeKey: "API-ключ (не Admin key)",
  connect: "Подключить среду",
  reconnect: "Переподключить среду",
  credentialsConfigured: "Данные туннеля сохранены",
  credentialsConfiguredBody: "Лаунчер повторно использует приватные Tunnel ID и API-ключ, уже сохранённые на этом устройстве.",
  replaceCredentials: "Заменить данные",
  keepCredentials: "Оставить сохранённые данные",
  mcpStepThree: "Подключите коннектор ChatGPT",
  mcpStepThreeBody: "Перед созданием коннектора включите Developer Mode в настройках ChatGPT. В ChatGPT Plugins создайте новый коннектор, выберите Tunnel, укажите созданный туннель, установите Authentication: None и используйте точное имя коннектора, показанное ниже. Откройте Permissions и выберите Allow all actions; Allow low-risk actions блокирует вызовы команд и патчей до того, как они попадут в эту среду выполнения. Внешняя среда Codex по-прежнему применяет sandbox и approvals. Затем проверьте среду выполнения.",
  manualMcpStepThreeBody: "Создайте отдельный коннектор, показанный ниже, с точным именем Codex Zero Risk и самостоятельно выбирайте его в каждом запросе Zero Risk. Для Zero Risk нужен собственный туннель и учётные данные, отдельные от автоматического режима. В этом режиме лаунчер никогда не анализирует DOM ChatGPT.",
  connectorMigrationNotice: "Переходите с Codex Native? Оставьте старый коннектор без изменений и создайте Codex Native2 как новый. Не переименовывайте и не обновляйте Codex Native. Проверка среды должна отклонять конфигурацию только со старым коннектором.",
  manualConnectorNotice: "Codex Zero Risk — отдельный коннектор. Выбирайте его самостоятельно перед отправкой каждого промпта Zero Risk; лаунчер намеренно не анализирует ChatGPT, чтобы проверить этот выбор.",
  openConnectors: "Открыть ChatGPT Plugins",
  connectorName: "Имя коннектора",
  verifyRuntime: "Проверить среду",
  checkingChatGptConnector: "Проверка коннектора ChatGPT",
  doctorProxyHealthy: "Responses proxy работает на {endpoint}",
  doctorTunnelBinaryInstalled: "Закреплённая версия openai/tunnel-client установлена",
  doctorTunnelKeyStored: "Ключ среды туннеля сохранён приватно",
  doctorTunnelRuntimeOwned: "Лаунчер управляет средой туннеля",
  doctorTunnelRuntimeReady: "Среда туннеля сообщает, что работает и готова",
  doctorConnectorAvailable: "Коннектор ChatGPT «{name}» доступен",
  activityTitle: "Активность среды выполнения",
  activitySubtitle: "Локальная диагностика. Перед отправкой другим экспортируйте безопасную для приватности копию; исходные логи остаются на этом устройстве.",
  recentActivity: "Последние события",
  noLogs: "Событий среды выполнения пока нет.",
  exportSafeLog: "Экспортировать безопасный лог",
  settingsTitle: "Настройки лаунчера",
  general: "Общие",
  launchAtLogin: "Запускать при входе в систему",
  launchAtLoginBody: "Поддерживать локальный маршрут Responses доступным до запуска Codex.",
  keepRunningOnClose: "Не останавливать сервер при закрытии окна",
  keepRunningOnCloseBody: "Скрывать лаунчер в системный трей, чтобы нативные модели и ChatGPT Web оставались доступными.",
  showDuringTurns: "Показывать браузер во время запросов",
  showDuringTurnsBody: "Показывать встроенный ChatGPT, когда активен браузерный запрос.",
  manualBiggerContextUnavailable: "Zero Risk уже использует фиксированный интервал compaction 3×. Его промпты остаются односоставными и отправляются вручную.",
  manualPromptTitle: "Отправьте этот запрос в ChatGPT",
  manualPromptInstruction: "Промпт уже скопирован в буфер обмена. Вставьте его в открытую вкладку ChatGPT, выберите нужные модель и уровень рассуждения, выберите коннектор Codex Zero Risk, отправьте промпт и затем подтвердите ниже.",
  manualPromptCopy: "Копировать промпт",
  manualPromptCancel: "Отменить запрос",
  manualPromptSent: "Отправлено",
  manualPromptWaiting: "Ожидание подключения Codex Zero Risk",
  manualPromptRunning: "ChatGPT выполняет задачу через среду Codex",
  manualPromptSeconds: "сек. осталось",
  language: "Язык",
  diagnostics: "Диагностика",
  runDoctor: "Запустить диагностику",
  cancelTurns: "Отменить активный запрос Codex",
  cancelTurnsBody: "Прервать активный HTTP-поток и связанный с ним сохранённый браузерный запрос ChatGPT.",
  turnsCancelled: "Активный запрос Codex отменён",
  uninstallIntegration: "Удалить интеграцию Codex",
  uninstallIntegrationBody: "Восстановить предыдущий маршрут моделей Codex и удалить приватную среду моста.",
  integrationRemoved: "Интеграция удалена; один раз перезапустите Codex",
  running: "Выполняется",
  complete: "Готово",
  failed: "Ошибка",
  healthy: "В норме",
  ...
  dismiss: "Закрыть",
  sessionReminderTitle: "Обновите сессию ChatGPT",
  sessionReminderBody: "Рекомендуется входить в аккаунт заново каждые два дня. Устаревшая сессия ChatGPT может прерывать запросы Web-моделей и работу MCP-инструментов.",
  logOut: "Выйти",
  checkingLocalRuntime: "Проверка локальной среды выполнения",
  doctorConfigValid: "Конфигурация корректна ({path})",
  doctorBrowserReady: "Встроенный браузер лаунчера авторизован и доступен (pid {pid})",
  doctorManualBrowserReady: "Встроенный браузер доступен для Zero Risk (pid {pid})",
  doctorCodexInstalled: "Нативный маршрут моделей Codex установлен",
  doctorRuntimeOwned: "Лаунчер управляет фоновой средой выполнения",
  doctorChromeFound: "Исполняемый файл Chrome найден: {path}",
  doctorLoginVerified: "Состояние входа в ChatGPT подтверждено данными авторизованного браузера",
  doctorMacServiceLoaded: "Фоновая служба macOS загружена",
  doctorMacTunnelRunning: "Служба туннеля macOS установлена, загружена и запущена",
};
export type Copy = typeof en;

export function copyFor(language: Language): Copy {
  if (language === "ru") return ru as Copy;
