...
const en = {
  ...
  manualPromptInstruction: "Paste this prompt into ChatGPT and send it with the Codex Zero Risk connector. If ChatGPT won't let you send because the selected model is unavailable or has reached its limit, select another available model in ChatGPT and copy/paste this prompt again. Only press Sent after ChatGPT accepts the message.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  manualPromptInstruction: "请将提示粘贴到 ChatGPT 并使用 Codex Zero Risk 连接器发送。若因所选模型不可用或已达用量上限而无法发送，可在 ChatGPT 中切换至其他可用模型后重新复制并粘贴此提示。请务必在 ChatGPT 成功接收消息后再点击已发送。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  manualPromptInstruction: "プロンプトを ChatGPT に貼り付け、Codex Zero Risk コネクタを使用して送信してください。選択中のモデルが利用不可または利用上限に達して送信できない場合は、ChatGPT 上で他の利用可能なモデルに切り替えてこのプロンプトを再度コピー・貼り付けしてください。ChatGPT がメッセージを受け付けた後にのみ「送信済み」を押してください。",
...
const ko: Record<keyof typeof en, string> = {
  ...
  manualPromptInstruction: "프롬프트를 ChatGPT에 붙여넣고 Codex Zero Risk 커넥터로 전송하세요. 선택한 모델을 사용할 수 없거나 사용 한도에 도달하여 전송할 수 없는 경우, ChatGPT에서 다른 사용 가능한 모델을 선택하고 이 프롬프트를 다시 복사하여 붙여넣으세요. ChatGPT가 메시지를 수락한 후에만 전송됨을 누르세요.",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  manualPromptInstruction: "請將提示貼到 ChatGPT 並使用 Codex Zero Risk 連接器傳送。若因所選模型無法使用或已達用量上限而無法傳送，可在 ChatGPT 中切換至其他可用模型後重新複製並貼上此提示。請務必在 ChatGPT 成功接收訊息後再點擊已傳送。",
