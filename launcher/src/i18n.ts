...
const en = {
  ...
  biggerContextParts: "Parts",
  biggerContextBody: "Warning: keeps small turns as one message, then splits large context across up to the configured number of messages (2-8, default 3). The final part starts the task without an extra request. Model context and compaction thresholds scale with the part count; restart Codex after changing it. Extra requests may increase rate limits or temporary cooldowns. Disabled by default.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  biggerContextParts: "分段数",
  biggerContextBody: "警告：较小回合仍作为一条消息发送；较大的上下文会按配置的段数拆分为多条消息（2-8 段，默认 3 段），最后一部分会直接开始任务，不会额外发送请求。模型上下文和压缩阈值随段数放大；更改后请重启 Codex。额外请求可能提高触发速率限制或临时冷却的概率。默认关闭。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  biggerContextParts: "パート数",
  biggerContextBody: "警告：小さなターンは 1 件のメッセージとして維持し、大きなコンテキストは設定された数（2～8 件、初期値 3 件）のメッセージに分割します。最後の部分から追加リクエストなしでタスクを開始します。モデルのコンテキスト上限とコンパクションしきい値はパート数に応じて拡大します。変更後は Codex を再起動してください。追加リクエストにより、レート制限や一時的なクールダウンが発生しやすくなる場合があります。初期設定では無効です。",
