...
const en = {
  ...
  devConnectorIsolationNotice: "Create a separate connector with the exact DEV name below. Keep your production connector unchanged so production and repository development can run side by side.",
  ...
  mcpStepTwoBody: "Choose a connector name, then paste the Tunnel ID and API key. The tunnel must belong to the same OpenAI account that will use the ChatGPT plugin. The key stays in private local storage and is never written to launcher logs.",
  ...
  connectorNameHint: "Choose any unique name you want to use for this ChatGPT connector. Existing configured names are preserved when reconnecting.",
  connectorNamePlaceholder: "My local tools",
  ...
  connectorMigrationNotice: "Upgrading from Codex Native? Leave the old connector untouched and create a new connector using the exact name shown below. Do not rename or refresh Codex Native. Verify runtime rejects a legacy-only setup.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  devConnectorIsolationNotice: "请使用下方准确的 DEV 名称新建独立连接器。保持生产连接器不变，让生产使用和仓库开发可以同时运行。",
  ...
  mcpStepTwoBody: "先选择连接器名称，再粘贴 Tunnel ID 和 API key。Tunnel 必须属于将使用该 ChatGPT 插件的同一 OpenAI 账户。密钥只保存在本机私有存储中，绝不会写入启动器日志。",
  ...
  connectorNameHint: "可为此 ChatGPT 连接器选择任意唯一名称。重新连接时会保留已配置的名称。",
  connectorNamePlaceholder: "我的本地工具",
  ...
  connectorMigrationNotice: "从 Codex Native 升级？请保留旧连接器不变，并使用下方显示的准确名称新建连接器。不要重命名或刷新 Codex Native。仅有旧连接器时，运行时验证会明确失败。",
