...
export function systemFirefoxExecutable(platform = process.platform): string | undefined {
  if (platform === "linux") return "/usr/bin/firefox";
  if (platform === "win32") {
    return join(process.env.PROGRAMFILES || "C:\\Program Files", "Mozilla Firefox", "firefox.exe");
  }
  return undefined;
}
