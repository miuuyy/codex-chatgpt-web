...
export function compileChatGptWebPrompt(
  ...
  const orchestrationContract = parsed._chatGptWebUltra === true && mode.localTools
    ? [
      ...
      "Start each worker with codex_exec by setting cmd to the exact reserved prefix [[CODEX_ULTRA_WORKER]] followed by one newline and the worker task, workdir to the intended workspace, yield_time_ms to 1000, and max_output_tokens to 20000. This is a bridge envelope, not a shell command: do not add an executable, model, sandbox, or service-tier argument. Do not discover or call spawn_agent or another collaboration tool.",
      "Start at most three independent workers before polling any of them. Give each a concrete non-overlapping task and retain every returned command session_id. The bridge intercepts the reserved envelope, pins the native Codex model reported by codex_bind_turn, forces read-only sandboxing, filters verbose child events, and leaves service tier at the user's default.",
      "After all workers have started, poll every returned session_id with codex_write_stdin until the process exits. A fast worker that returns a complete filtered result inline needs no poll. A worker counts only when exit_code is zero and its filtered result contains both worker_task_id and a non-empty final_report.",
