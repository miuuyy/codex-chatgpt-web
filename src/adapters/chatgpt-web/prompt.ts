...
export function compileChatGptWebPrompt(
  ...
  const orchestrationContract = parsed._chatGptWebUltra === true && mode.localTools
    ? [
      ...
      "Start each worker with codex_exec by setting cmd to the exact reserved prefix [[CODEX_ULTRA_WORKER]] followed by one newline and the worker task. This is a bridge envelope, not a shell command: do not add an executable, model, sandbox, or service-tier argument. Do not discover or call spawn_agent or another collaboration tool.",
